...
def test_get_text_animations_names_are_compatible_with_add_captions():
    """get_text_animations 返回的名称应可直接用于 add_captions 的动画参数。"""
    for animation_type in ("in", "out", "loop"):
        animations = get_text_animations(mode=0, type=animation_type)
        assert animations, f"{animation_type} 动画列表不应为空"

        for animation in animations:
            mapped_enum = map_animation_name_to_enum(
                animation_name=animation["name"],
                animation_type=animation_type,
            )
            assert mapped_enum is not None, (
                f"动画 '{animation['name']}' ({animation_type}) 无法映射到 add_captions"
            )


def test_get_text_animations_mode_filter_works():
    """mode 过滤逻辑应正确区分 VIP 与免费动画。"""
    all_animations = get_text_animations(mode=0, type="in")
    vip_animations = get_text_animations(mode=1, type="in")
    free_animations = get_text_animations(mode=2, type="in")
...
def test_get_text_animations_keyword_filter_works():
    ...
    filtered = get_text_animations(mode=0, type="in", keyword=f"  {keyword.lower()}  ")
    assert filtered, "关键字过滤后结果不应为空"
    assert all(keyword.lower() in item["name"].lower() for item in filtered)

    filtered_with_blank_keyword = get_text_animations(mode=0, type="in", keyword="   ")
